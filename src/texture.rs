...
impl Builder<()> {
  ...
  pub fn with_context(self, context: &sys::Context) -> Builder<sys::Context> {
    let Self {
      context: (),
      mipmaps,
    } = self;
...
impl Builder<sys::Context> {
  ...
  fn apply_pre_texture_state(&self, texture: &Texture) {
    ...
    // TODO: Probably not the full story. Also likely not really
    //       specific to the bound texture.
  ...
  /// Create a `Texture` suitable for use as a depth map.
  ...
  /// Create a new `Texture` from the provided image data.
